        ...
        <div v-else>
            ...
            <div class=" mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                <v-card v-for="post in posts" :key="post.id" 
                    :title="post.title.split('').slice(0, 50).join('')"
                    :text="post.content.split('').slice(0, 100).join('') + '.....'"
                    elevated>
                    <v-card-actions>
                        <v-btn :to="{ name: 'postedit', params: { id: post.id } }">Read More</v-btn>
                        <v-btn v-if="auth.authPermissions?.includes('delete-post')" @click="store.deletePost(post.id)"
